...
- **[FCIS failure precedence]:** Preserve existing failure ordering by checking settings/auth preconditions before repository reads, and validating cheap domain preconditions before expensive context loads; back this with tests that assert skipped dependency calls.
...
- **[Renderer conductor shell extraction]:** For `stores/sessions.ts` conductor-loop coverage, move response-code branching into a pure shell planner (`lib/shell/renderer/conductor-turn-shell-plan.ts`) and keep the store as interpreter-only; this makes branch matrix testing fast and lifted shell lines from 32.3% to 41.3%.
- **[Conductor mode transport contract]:** When introducing new session conductor settings, update both `electron/handlers/session-state-handlers.ts` and `electron/handlers/conductor.ts` IPC schemas together so session command payloads remain aligned across boundaries.
- **[Settings catalog ownership]:** Use `settings:getModelCatalog` as the renderer boundary for model loading; keep cache TTL and API-key preconditions in `electron/handlers/settings.ts` so renderer stores only reflect returned snapshot state.
- **[IPC-wide diagnostics seam]:** Use `registerPrivilegedIpcHandle` in `electron/lib/security/privileged-ipc.ts` as the canonical place for one wide log event per invoke (`channel`, `duration_ms`, `outcome`, `failure_code`) instead of scattering per-handler transport logs.
