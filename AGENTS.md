...
- **[Conductor mode transport contract]:** When introducing new session conductor settings, update both `electron/handlers/db.ts` Zod schemas and `electron/handlers/conductor.ts` IPC args together; otherwise unknown keys are stripped at the DB boundary and the renderer silently falls back to default mode.
