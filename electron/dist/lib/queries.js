...
async function ensureDb() {
    if (!dbInitPromise) {
        dbInitPromise = (0, db_js_1.initializeDatabase)().then(() => { });
    }
    await dbInitPromise;
}
...
async function createSession(data) {
    const id = generateUUID();
    const now = new Date().toISOString();
    await runQuery(`INSERT INTO sessions (id, title, problem_description, output_goal, status, token_count, cost_estimate, created_at, updated_at)
...
async function createMessage(data) {
    const id = generateUUID();
    const now = new Date().toISOString();
    await runQuery(`INSERT INTO messages (id, session_id, persona_id, content, turn_number, token_count, created_at)
