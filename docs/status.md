...
- Councils runtime milestone now includes transcript/message persistence, runtime briefing snapshot persistence, and generation orchestration skeleton commands for Manual + Autopilot turn progression.
...
- Known gaps/follow-ups: generation is still placeholder/fake-provider driven (no final prompt polish, no export, no retry UX); Councils list/view/editor UX is wired but still needs deeper interaction/accessibility polish.
...
| R2.1, R2.2, R2.3, R2.4, R2.6, R2.7, R2.8, R2.9, R2.11, R2.20, R2.21, R2.22, R2.23, R2.24 | In progress | Councils foundation supports create/edit/delete/archive/restore/list with required-field validation, mode immutability, tag checks, archived filtering, search/sort/pagination, persisted member references, runtime lifecycle fields, transcript persistence baseline, and archive blocking while Autopilot is running and not paused. | Main slice in `src/main/features/councils/slice.ts`; IPC handlers in `src/main/features/councils/ipc-handlers.ts`; IPC wiring in `src/main/ipc/register-ipc.ts`; renderer list/editor/view in `src/renderer/App.tsx`; DTO/contracts in `src/shared/ipc/dto.ts` and `src/shared/ipc/validators.ts`; persistence in `src/main/services/db/sqlite-persistence-service.ts` + `src/main/services/db/migrations/0002_councils_foundation.sql`, `src/main/services/db/migrations/0003_councils_runtime_foundation.sql`, and `src/main/services/db/migrations/0004_council_runtime_messages.sql`. | Runtime-dependent council rules (`R2.5`, `R2.10`, `R2.12`-`R2.19`) still need full production-provider generation, export, and final Council View config/member tooling. | `tests/integration/councils-handlers.integration.spec.ts`, `tests/integration/councils-ipc.contract.integration.spec.ts`, `tests/integration/sqlite-persistence.integration.spec.ts`, `scripts/diagnostics/scenarios/councils-ui.json` |
| R3.1, R3.2, R3.3, R3.6, R3.7, R3.8, R3.9, R3.10, R3.11, R3.25, R3.26 | In progress | Council View now renders persisted transcript baseline and runtime briefing snapshot, exposes manual speaker selection + trigger action, conductor message injection, autopilot next-turn command, and cancellation that discards in-flight output. | Runtime read/command methods in `src/main/features/councils/slice.ts` + IPC wiring in `src/main/features/councils/ipc-handlers.ts`, `src/main/ipc/register-ipc.ts`, and `src/preload/index.ts`; Council View UI in `src/renderer/App.tsx`; persistence in `src/main/services/db/sqlite-persistence-service.ts`. | Generation remains skeleton only (fake AI boundary, no provider adapters/prompt polish, no export/retry UX, no navigation-confirm pause/cancel flow for `R3.12`, no full briefing/goal evaluator for `R3.13`-`R3.17`). | `tests/integration/councils-handlers.integration.spec.ts`, `tests/integration/councils-ipc.contract.integration.spec.ts`, `tests/integration/sqlite-persistence.integration.spec.ts` |
| R4.1, R4.2, R4.3, R4.4, R4.5, R4.6, R4.7, R4.8, R4.9, R4.10, R4.11, R4.12, R4.13, R4.14, R4.15, R4.16, R4.17, R4.18, R4.19, R4.20, R4.21 | In progress | Settings slice supports provider cards (Gemini/Ollama/OpenRouter), test-before-save gate, save-triggered catalog refresh, manual refresh gating, grouped model picker, global default selection, invalid-config detection for null/default resolution, webContents-scoped snapshot cleanup hooks, renderer unload prompt when there are unsaved draft changes, dynamic Ollama model discovery from local `/api/tags`, SQLite-backed persistence for provider/global-default settings, and council start/resume invalid-config blocking in runtime commands. | Shared pure rules in `src/shared/domain/model-ref.ts`; settings slice orchestration in `src/main/features/settings/slice.ts` (including Ollama model fetch dependency + refresh path); strict IPC handlers in `src/main/features/settings/ipc-handlers.ts` + lifecycle wiring in `src/main/ipc/register-ipc.ts` and `src/main/index.ts`; council runtime checks in `src/main/features/councils/slice.ts`; preload bridge in `src/preload/index.ts`; renderer page in `src/renderer/App.tsx`; SQLite persistence in `src/main/services/db/sqlite-persistence-service.ts`. | Runtime-failure-triggered refresh (`R4.16`) and all remaining Council View invalid-config UX (`R4.11`, `R4.20`, `R4.21`) still need deeper transcript/runtime error-path integration. | `tests/unit/model-resolution.spec.ts`, `tests/unit/ipc-validator-shapes.spec.ts`, `tests/integration/providers-handlers.integration.spec.ts`, `tests/integration/providers-ipc.contract.integration.spec.ts`, `tests/integration/councils-handlers.integration.spec.ts`, `tests/integration/councils-ipc.contract.integration.spec.ts`, `tests/integration/sqlite-persistence.integration.spec.ts` |
...
| R7.1, R7.2 | In progress | Main-process keychain adapter uses keytar wiring for provider secret writes while preserving secret refs in provider config responses; settings/agents/councils foundations now persist locally in SQLite under userData, including council transcript messages and runtime briefing snapshot records. | Keychain adapter in `src/main/services/keychain/keytar-keychain-service.ts`; SQLite persistence and migrations in `src/main/services/db/sqlite-persistence-service.ts` + `src/main/services/db/migrations/0001_initial_schema.sql`, `src/main/services/db/migrations/0002_councils_foundation.sql`, `src/main/services/db/migrations/0003_councils_runtime_foundation.sql`, and `src/main/services/db/migrations/0004_council_runtime_messages.sql`; wired through `src/main/ipc/register-ipc.ts`, `src/main/features/settings/slice.ts`, `src/main/features/agents/slice.ts`, and `src/main/features/councils/slice.ts`. | Provider integration for runtime generation remains placeholder/fake and still needs production adapters. | `tests/unit/keytar-keychain-service.spec.ts`, `tests/integration/providers-ipc.contract.integration.spec.ts`, `tests/integration/sqlite-persistence.integration.spec.ts` |
...
| U3.1, U3.2, U3.3, U3.4, U3.5, U3.6, U3.7, U3.8, U3.9, U3.10, U3.11 | In progress | Councils tab renders list rows with title/topic/mode/runtime status/model/tags, archived + invalid-config badges, search/tag/archive/sort controls, pagination/load-more, New Council CTA, and Open/Edit row actions that now route into Council View + editor flows. | `src/renderer/App.tsx`, `src/main/features/councils/slice.ts`, `src/main/features/councils/ipc-handlers.ts`. | Overflow menu parity (Export/Archive/Delete in one menu), keyboard row activation polish, and tooltip-driven disable explanations still need completion. | `scripts/diagnostics/scenarios/councils-ui.json`, `tests/integration/councils-handlers.integration.spec.ts` |
...
| U12.3 | In progress | Autopilot runtime controls now expose Pause and Resume in Council View once an Autopilot council is started. | `src/renderer/App.tsx`, `src/main/features/councils/slice.ts`. | Start modal limits (`U12.1`, `U12.2`) and runtime placeholder/error UX (`U12.4`-`U12.6`) remain pending. | `tests/integration/councils-handlers.integration.spec.ts`, `tests/integration/councils-ipc.contract.integration.spec.ts` |
...
| B1, B2, B3, B4 | In progress | SQLite persistence service is implemented with userData database path wiring, ordered migration application, schema migration tracking, foreign-keys pragma, persisted councils/member references, persisted council runtime lifecycle fields (started/paused/turn-count), and persisted council messages/runtime briefing snapshots. | `src/main/services/db/sqlite-persistence-service.ts`, `src/main/services/db/migrations/0001_initial_schema.sql`, `src/main/services/db/migrations/0002_councils_foundation.sql`, `src/main/services/db/migrations/0003_councils_runtime_foundation.sql`, `src/main/services/db/migrations/0004_council_runtime_messages.sql`, `src/main/ipc/register-ipc.ts`, `scripts/verify-migrations.ts`. | Advanced orchestration constraints and export semantics remain future slices. | `tests/integration/migration-verification.integration.spec.ts`, `tests/integration/sqlite-persistence.integration.spec.ts` |
...
- `ipc-validator-shapes.spec.ts` (`tests/unit/ipc-validator-shapes.spec.ts`) -> A3, R4.6, R4.8, R4.17, R1.1, R1.2, R2.1, R2.3, R2.7, R3.1, R3.7, R3.8, R6.1
...
- `sqlite-persistence.integration.spec.ts` (`tests/integration/sqlite-persistence.integration.spec.ts`) -> B1, B2, B3, R7.1, R2.1, R3.3, R3.7, R3.8
...
- `councils-handlers.integration.spec.ts` (`tests/integration/councils-handlers.integration.spec.ts`) -> R2.1, R2.2, R2.3, R2.4, R2.6, R2.7, R2.8, R2.9, R2.11, R2.20, R2.21, R2.22, R2.23, R2.24, R3.3, R3.7, R3.8, R3.9, R3.10, R3.11, R3.25, R3.26, R6.1, R6.2, R6.3, R6.4, R6.6, U12.3, U13.1
- `councils-ipc.contract.integration.spec.ts` (`tests/integration/councils-ipc.contract.integration.spec.ts`) -> A3, R2.1, R2.3, R3.1, R3.7, R3.25, R3.26
...
- Current in-progress items:
  - Councils runtime now includes transcript persistence + runtime briefing snapshots and generation-orchestration skeleton commands (manual trigger, conductor injection, autopilot next-turn, cancel-discard).
...
- Next implementation step:
  - Replace fake AI boundary with real provider adapters and complete full briefing/goal evaluation, retry UX, export flow, and navigation-confirm pause/cancel behavior.
